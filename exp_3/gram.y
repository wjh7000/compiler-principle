...
%}
	...
	%token  LT EQ GE LE NEQ SLP SRP SEMI
token REL
...
S: ID EQ E	   { copyaddr(&$1, "ID"); 
 ...
 | WHILE M C DO M S		{ backpatch(list, $3.nextlist, $2.instr);
...
C:    | E REL E        { $$.truelist = new_instrlist(nextinstr(list));
                          $$.falselist = new_instrlist(nextinstr(list)+1);
						  gen_if(list, $1, $2.oper, $3);
                          gen_goto_blank(list); }
...
F: SLP E SRP	{ copyaddr_fromnode(&$$, $1); }
